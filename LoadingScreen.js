export class LoadingScreen {
  constructor(options = {}) {
    ...
    // All 5 echoes.
    // Each echo uses one dormant/outline PNG.
    // The awakened color is added in code using the same image,
    // keeping both layers matched and aligned.
    this._echoKeys = ['azure', 'crimson', 'flowerYellow', 'medive', 'ribbon'];

    // Colors used for each awakened echo.
    // These are deeper, more saturated shades for a stronger look.
    this._echoColors = {
      azure:        '#00B4E6', 
      crimson:      '#E0102B', 
      flowerYellow: '#FFC400', 
    ...
    // Builds a tinted version of `img` using the given color.
    // Keeps the original alpha shape and fills it with the new color.
  ...
  // Pauses and hides the .ls-video element itself instead of relying on
  // the overlay or canvas to cover it. This prevents the video from
  // flashing through when the canvas fades in or out.
  _hideLsVideo() {
  ...
  _runTransitionSequence(barWrap) {
    // The video was still visible behind the canvas during the transition.
    // When the canvas faded out, the video could briefly flash through.
    // Pause and hide the video first so it stays hidden until the overlay is gone.
    this._hideLsVideo();
    ...
    const PHASE_A_MS = 584;   
    const PHASE_C_MS = 964; 
    ...
    // All 5 Echo icons — centered horizontally with staggered awakening
    const echoKeys   = this._echoKeys;
    const echoImages = this._echoImages;

    // Stagger config: each echo starts awakening at a different time
    // Echo 0 starts first, and echo 4 starts last
    const PHASE_B_LEN = PHASE_B_END - PHASE_B_START;

    // Each echo awakens one at a time.
    // fillDuration: time for one echo to fully fill
    // staggerStep: delay before the next echo starts
    const fillDuration = PHASE_B_LEN * 0.25;                       
    const staggerStep  = (PHASE_B_LEN - fillDuration) / (echoKeys.length - 1); 
    ...
    function drawAllEchoes(ctx, W, H, t) {
      ...
      for (let i = 0; i < COUNT; i++) {
        ...
        // Layer 1: Dormant (outline) — always visible and keeps its aspect ratio
        ...
        // Layer 2: Awakened (tinted) — revealed left-to-right for each echo
        // `imgs.tinted` uses `imgs.dormant` as its base, so they have the same shape
        // Use `dormantRect` for both to keep the outline and color aligned
        if (imgs.tinted && dormantRect && fillProgress > 0) {
          ...
          ctx.clip();
          // Boost saturation and contrast so the awakened color stands out more
