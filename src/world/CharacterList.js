  ...
  export class CharacterList {
    ...
    _hydrateSVGs() {
      ...
      const loads = placeholders.map((ph) => {
        ...
        return fetch(src)
          ...
          .then((txt) => {
            ...
            // Cache the animatable parts once so _startBreatheForWrap and
            // _stopBreatheForWrap do not have to search the SVG again.
            ph._svgParts = {
      ...
      // Coordinated reveal: wait for all previews (SVG + iframes) to finish
      // loading before fading them in, with a small 70ms stagger.
      // Iframe loads are also staggered by 150ms so the three pages
      ...
      const readiness = wraps.map((wrap, myIndex) => new Promise((resolve) => {
        ...
        if (iframe) {
          ...
          // Stagger each iframe load by 150ms so the three pages
          // do not load at the same time.
    ...
    show() {
      ...
      this._buildDOM();
      this._bindEvents();
      // Two rAFs: wait for the layout to render before starting the heavier
      // work (SVG fetch/parse and iframe loads).
    ...
    _updateSelectedInfo(index) {
      ...
      // - .torso -> subtle breathing using scaleY
      // - .iris-left/right -> subtle pulse that follows the breathing
      // - #eyelid-left/right -> blinking, scaleY(0) = open, scaleY(1) = closed
      // Each card has its own timers in wrap._breatheTimers
      // Only the focused card runs these animations, so the others stay idle.
      // PERF: The SVG parts are cached in wrap._svgParts during _hydrateSVGs(),
      // so we don't have to search the SVG again every time the focus changes.
    ...
    // Controller: only starts the animation for the currently focused card
    // Other cards stay idle until they are focused (see _focusCard)
    ...
    // back button.
      ...
      // PERF: Reduced the key-repeat cooldown from 280ms to 120ms
      // e.repeat is also allowed so holding the arrow keys feels smoother
      _onKey(e) {
      ...
      _focusCard(index) {
        ...
        // PERF: Removed the old 60ms setTimeout before scrolling
        // Rapid movement kept canceling and rescheduling it, which made
        // the scroll feel like it was lagging behind. Now it uses a single
        // rAF to handle the scroll without the extra delay.
